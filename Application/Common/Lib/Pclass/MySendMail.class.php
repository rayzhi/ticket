...
class MySendMail {
    ...
    protected $_sendServer;  // 邮件传输代理服务器地址
    protected $_port = 25;  // 邮件传输代理服务器端口
    protected $_from;  // 发件人
    protected $_to;  // 收件人
    protected $_subject;  // 主题
    protected $_body;  // 邮件正文
    protected $_isHTML = false;  // 是否是HTML格式的邮件
    protected $_socket;  // socket资源
    ...
    public function __construct($from="", $to="", $subject="", $body="", $server="", $username="", $password="",$isHTML="", $port="") {
        ...
        if(!empty($to)) $this->_to = $to;
        if(!empty($subject)) $this->_subject = $subject;
        if(!empty($body)) $this->_body = $body;
        if(!empty($isHTML)) $this->_isHTML = $isHTML;
        if(!empty($server)) $this->_sendServer = $server;
    ...
    */
    public function setMailInfo($config) {
        ...
        if(isset($config['isHTML'])) $this->_isHTML = $config['isHTML'];
        return true;
    }
    ...
    */
    public function error(){
        if(!isset($this->_errorMessage)) $this->_errorMessage = "";
